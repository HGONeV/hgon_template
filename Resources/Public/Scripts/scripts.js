...
$('[data-target]').click(function(e){
  ...
  TweenMax.to(window, 1, {scrollTo:{y:$target, offsetY:diff, autoKill:false}, ease:Power2.easeInOut});
});
...
$('.main .main__header h1 .headline:empty').remove();

// wrap embed videos for responsive
var $embeds = $('iframe[src*="youtube"], iframe[src*="vimeo"], iframe[src*="google.com/maps"');
if ($embeds.parents('.video').length <= 0) {
  $embeds.wrap('<div class="video"></div>');
}
...
(function($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    open: function(e) {
      ...
      TweenMax.to(self._settings.$lightbox, 0.35, {
        autoAlpha: 1,
        className: '+=is-open',
    ...
    next: function(e) {
      ...
      TweenMax.to(self._settings.$currentImg, 0.5, {
        autoAlpha: 0,
        ease: self.options.ease
      }, 0);
      TweenMax.to(self._settings.$nextImg, 0.5, {
        autoAlpha: 1,
        ease: self.options.ease
      }, 0);
    ...
    prev: function(e) {
      ...
      TweenMax.to(self._settings.$prevImg, 0.5, {
        autoAlpha: 1,
        ease: self.options.ease
      }, 0);
    ...
    setCurrentImage: function(direction) {
      ...
      if (direction === 'next') {
        ...
        if( index < 0 ) { index = this._settings.lightboxSize - 1; }
      } else {
        index = this._settings.lightboxIndex + 1;
        if( index >= this._settings.lightboxSize ) { index = 0; }
      }
 ...
 *
 * Authors: Patrick Schumacher <schumacher@helllicht.com>, Christian Dilger <dilger@helllicht.com>
 *
 * based on jQuery lightweight plugin boilerplate
 * Original author: @ajpiano
 * Further changes, comments: @addyosmani
...
;(function($, window, document, undefined) {
  ...
  var pluginName = "helllformValidator",
    defaults = {
      key: "value"
    };
          ...
          $.ajax({
            ...
            success: function(response) {
              var key = $(el).attr('name'),
                  ...
                  $el_wrapper = $(el).parents('.form__field'),
                  $required_star = $el_wrapper.find('.is-required');
                  $el_error = $el_wrapper.find('.form__error');
              if ( response._valid != true ) {
                $(el).addClass('error');
                $required_star.addClass('error');
                if ( key != message ) { $el_error.addClass('form__error--show').text( message ); }
              } else {
                $(el).removeClass('error');
                $(el).parent().removeClass('error');
                $required_star.removeClass('error');
                if ( key != message ) { $el_error.removeClass('form__error--show'); }
  ...
  Plugin.prototype = {
    ...
    open: function(e) {
      ...
      if ($(this).hasClass('js-set-mdl')) {
        ...
        self._settings.$modal.css({'left' : xPos + 'px', 'top' : yPos + 'px'});
      }

      self.checkFullscreen();

      TweenMax.to(self._settings.$modal, 0.35, {
        autoAlpha:1,
        className:'+=is-open',
        ease: self.options.ease
      }, 0);
      TweenMax.to(self._settings.$layer, 0.35, {
        autoAlpha: 1,
        ease: self.options.ease
      }, 0);
...
(function($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    open: function(e) {
      ...
      tlNavOpen
        .to(self._settings.$layer, 0.25, {
          autoAlpha: 1,
          ease: self.options.ease
        }, 0)
        .to(self._settings.$mobileNav, 0.5, {
          x: '0%',
          ease: self.options.ease
        }, 0);
...
(function($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    // initialize slides, set first slide as active
    initSlides: function() {
      if ( this._settings.mode == 'fade' ) {
        TweenMax.set(this._settings.$sliderItems, { autoAlpha: 0, });
        TweenMax.set(this._settings.$sliderItems.first(), { autoAlpha: 1, className: '+=active' });
      } else {
        TweenMax.set(this._settings.$sliderItems, { 'x': '100%' });
        TweenMax.set(this._settings.$sliderItems.first(), { 'x': '0%', className: '+=active' });
      }
    },
    ...
    // initialize pills, set first pill as active
    initPills: function() {
      var self = this;
      ...
      // no pills if only 1 slide
      if ( this._settings.sliderLength <= 1 ) { self._settings.$sliderPills.remove(); }

      // disable click on slide mode
      if ( self._settings.mode != 'fade' ) {
        self._settings.$sliderPills.find('a').css({'cursor':'default'}).on('click', function(e) {
          e.preventDefault();
        });
      }
    },
    ...
    initAutoplay: function() {
      var self = this;
      if ( this._settings.sliderLength == 1 || this._settings.autoplay !== true ) { return; }
    ...
    next: function(e) {
      ...
      var $activeSlide = self._settings.$sliderItems.eq(self._settings.sliderCount),
          $nextSlide = self._settings.$sliderItems.eq(self._settings.sliderCount + 1),
          tlSlideNext = new TimelineMax({onComplete:function(){ self._settings.$btnPrevSlide.removeClass('is-disabled'); }});
      ...
      if ( self._settings.mode == 'fade' ) {
        tlSlideNext
          .to($activeSlide, 0.5, { autoAlpha: 0, ease: self._defaults.ease }, 0)
          .to($nextSlide, 0.5, { autoAlpha: 1, ease: self._defaults.ease }, 0);

      // slide mode
      } else {
        tlSlideNext
          .to($activeSlide, 0.75, { x: '-100%', className: '-=active', ease: self._defaults.ease }, 0)
          .to($nextSlide, 0.75, { x: '0%', className: '+=active', ease: self._defaults.ease }, 0);
      }
    ...
    prev: function(e) {
      ...
      var $activeSlide = self._settings.$sliderItems.eq(self._settings.sliderCount),
        $nextSlide = self._settings.$sliderItems.eq(self._settings.sliderCount - 1),
        tlSlidePrev = new TimelineMax({onComplete:function(){ self._settings.$btnNextSlide.removeClass('is-disabled'); }});
      ...
      // slide mode
        ...
        tlSlidePrev
          .to($activeSlide, 0.75, { x: '100%', className: '-=active', ease: Power3.easeInOut }, 0)
          .to($nextSlide, 0.75, { x: '0%', className: '+=active', ease: Power3.easeInOut }, 0);
      }
    ...
    // necessarry for looping and pills slideTo(2)
    slideTo: function(e) {
      var slideTo = null;
      if ( e.target) {
        e.preventDefault();
      ...
      var $activeSlide = (e.target) ? self._settings.$sliderItems.eq(self._settings.sliderCount) : self._settings.$sliderItems.eq(slideTo-1),
          $targetSlide = self._settings.$sliderItems.eq(slideTo),
          tlSlideTo = new TimelineMax();


      // fade mode
      if ( self._settings.mode == 'fade' ) {
        tlSlideTo
          .to( self._settings.$sliderItems, 0.5, { autoAlpha: 0, ease: self._defaults.ease }, 0 )
          .to( $targetSlide, 0.5, { autoAlpha: 1, ease: self._defaults.ease }, 0 );

      // slide mode
      } else {
        if ( slideTo == 0 ) {
          self._settings.$sliderItems.css({'transform': 'translate(100%,0) matrix(1, 0, 0, 1, 0, 0)' });
          tlSlideTo
            .to( self._settings.$sliderItems, 0.85, { x: '100%', className: '-=active', ease: Power3.easeInOut }, 0 )
            .to( $targetSlide, 1, { x: '0%', className: '+=active', ease: Power3.easeInOut }, 0 );
          ...
          self._settings.$sliderItems.css({'transform': 'translate(-100%,0) matrix(1, 0, 0, 1, 0, 0)' });
          tlSlideTo
            .to( $activeSlide, 0.85, { x: '-100%', className: '-=active', ease: Power3.easeInOut }, 0 )
            .to( $targetSlide, 1, { x: '0%', className: '+=active', ease: Power3.easeInOut }, 0 );
        }
      }

      // set counter and pills
    ...
    setCurrentSlide: function(num) {
      var self = this;
      // set slide height if autoheight
      if ( this._settings.autoheight === true ) {
        var sliderHeight = new TimelineMax();
        sliderHeight.to( $(this.element), 0.85, { height: this._settings.$sliderItems.eq(num).height() + 'px', ease: Power3.easeInOut }, 0 );
      }
  ...
  Plugin.prototype = {
    ...
    buildNav: function() {
      ...
      self._settings.$sections.each(function(i) {
        ...
        var sectionName = $(this).attr('data-pagenav'),
          sectionSlug = $(this).attr('id');
 ...
 *
 * @todo: Hash und Select kombinieren
 *
 */
...
(function($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    fadeOut: function(self) {
      ...
      TweenMax.to(self._settings.$tab, 0.5, {
        autoAlpha: 0,
        className: '-=active',
        ease: self.options.ease
      }, 0);
    ...
    fadeIn: function(self) {
      ...
      TweenMax.to(self._settings.$currentTab, 0.5, {
        autoAlpha: 1,
        className: '+=active',
        ease: self.options.ease
      }, 0);
    ...
    setTabContainerHeight: function(self) {
      ...
      TweenMax.to(self._settings.$tabs, 0.5, {
...
$(function() {
  var Message = {
    ...
    open: function(e) {
      e.preventDefault();
      var msgType = $(this).data('msg'),
          $msg = $('.js-msg-' + msgType);
      TweenMax.to(Message.$layer, 0.35,{autoAlpha:1, ease:Power3.easeInOut}, 0);
      TweenMax.to($msg, 0.35,{top:'50%', autoAlpha:1, ease:Power3.easeInOut}, 0);
    },

    close: function(e) {
      e.preventDefault();
      $msg = $('.js-msg');
      TweenMax.to(Message.$layer, 0.35,{autoAlpha:0, ease:Power3.easeInOut}, 0);
      TweenMax.to($msg, 0.35,{top:'51%', autoAlpha:0, ease:Power3.easeInOut}, 0);
    },
...
$('.btn.is-animated-submit').click(function(e){
  ...
  var $loadIcon = $(this).find('.icon-loader'),
      $doneIcon = $(this).find('.icon-done'),
      $errorIcon = $(this).find('.icon-error'),
      tlSubmit = new TimelineMax();

  tlSubmit
   .set($loadIcon, {autoAlpha:0, right:'10px'})
   .set($doneIcon, {autoAlpha:0, right:'32px'})
   .to($(this), 0.25, {paddingRight:'3em', ease:Power3.ease}, 0)
   .to($loadIcon, 0.1, {autoAlpha:1, ease:Power3.ease}, 0.25)
   .to($loadIcon, 0.1, {autoAlpha:0, right:'0px', ease:Power3.ease, onComplete:sendForm()}, 1.5);

   if (send === false) {
     TweenMax.to($errorIcon, 0.1, {autoAlpha:1, right:'10px', delay:1.75, ease:Power3.ease});
   } else {
     TweenMax.to($doneIcon, 0.1, {autoAlpha:1, right:'10px', delay:1.75, ease:Power3.ease});
   }
...
for ( var i in carouselOptions ) {
    var selector = '.js-carousel.carousel-' + i;
    var carousel = carouselOptions[i];
    carousel.swipeAngle = false;
    if ( !carousel.speed ) { carousel.speed = 400; }

    if ( document.querySelector(selector) ) {
        var allContainer = document.querySelectorAll(selector);
        for ( var x = 0; x < allContainer.length; ++x) {
            var item = allContainer[x];
            carousel.container = '#' + item.id;
            if ( typeof carousel.responsive !== 'undefined' && typeof item.dataset.items !== 'undefined' ) {
                carousel.responsive[768].items = item.dataset.items;
            }
            carousels['#' + item.id] = tns(carouselOptions[i]);
        }
    }
}
